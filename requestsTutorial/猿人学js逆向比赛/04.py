...
def request_page(page_num:int):
    url = f"http://match.yuanrenxue.com/api/match/4?page={str(page_num)}"
    r = requests.get(url=url)
    return r.json()

def md5_str(key:str, value:str) -> str:
    b64 = base64.b64encode((key+value).encode("ascii")).decode("ascii").replace("=", "")
    m = hashlib.md5()
    m.update(b64.encode("ascii"))
    # print(m.hexdigest())
    return m.hexdigest()

def __parse_td(td:str)-> list:
    td_list = re.findall("(<td>)(.*?)(</td>)", td)
    # print(len(td_list),td_list)
    return td_list

def parse_src_css_class(td:str, m_check:str)->list:
    src_list = []
    td_list = __parse_td(td)
    for _, td, _ in td_list:
        temp = []
        src = re.findall('<img src="(.*?)" class="', td)
        md = re.findall('class="img_number (.*?)" style="', td)
        css = re.findall('style="left:(.*?)px">', td)
        for index, m in enumerate(md):
            if m != m_check:
                temp.append([css[index], m, src[index]])
        src_list.append(temp)
    # [[(src, m, css), (src1, m1, css1)], [(src, m, css), (src1, m1, css1)]]
    # 将css：left属性进行排序
    res_list = []
    pp = []
    for td in src_list:
        ...
        for index, t in enumerate(td):
            tmp.append([int(t[0]) + (index * 11), t[1], t[2]])
            p.append([int(t[0]), int(t[0]) + (index * 11)])
        new_tmp = sorted(tmp, key=lambda t: t[0])
        res_list.append(tmp)
        pp.append(p)
    print(pp)
    # 解析成具体的数字 排列
...
def main():
    ...
    for i in range(1, 2):
        ...
        json = request_page(i)
        # 2.计算需要校验的md5值
        m_check = md5_str(json.get("key"), json.get("value"))
        # 3.解析td
        td_list = parse_src_css_class(json.get("info"), m_check)
        # 4.每个td中有56个图片，需要判断哪些图片是显示出来的
        result += sum(td_list)
    print(result)
